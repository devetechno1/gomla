...
class CartItemWidget extends StatelessWidget {
  ...
  const CartItemWidget({super.key, required this.cart, required this.cartIndex, required this.isAvailable, required this.addOns, this.variation});
                            ...
                            children: [
                              ClipRRect(
                                ...
                                child: CustomImage(
                                  ...
                                  height: ResponsiveHelper.isDesktop(context) ? 90 : 70, width: ResponsiveHelper.isDesktop(context) ? 90 : 45, fit: BoxFit.cover,
                                ),
                              ),
                                ...
                                return GetBuilder<ItemController>(
                                  ...
                                  return Row(children: [
                                    ...
                                    TextButton(
                                      ...
                                      child:  Text('delete_from_cart'.tr),
                                    ),
                                    // QuantityButton(
                                    //
                                    ...
                                    QuantityButton(
                                      // onTap:  () {
                                      //   print("ffff=====${cart.quantity!}");
                                      //    print("ffff=====${cartController.cartList[itemController.cartIndex].variation![itemController.currentIndex].maxVariations}");
                                      //   if(cart.quantity! +1  <= cartController.cartList[itemController.cartIndex].variation![itemController.currentIndex].maxVariations!) {
                                      //     Get.find<CartController>().forcefullySetModule(Get.find<CartController>().cartList[cartIndex].item!.moduleId!);
                                      //     Get.find<CartController>().setQuantity(true, cartController.currentIndex, cart.stock, cart.quantityLimit);
                                      //   }else{
                                      //     showCustomSnackBar("Maximum order is ${cartController.cartList[itemController.cartIndex].variation![itemController.currentIndex].maxVariations}");
                                      //   }
                                      // },
                                        ...
                                        if (cart.quantity! <=cart.stock!) {
                                          ResponsiveHelper.isMobile(context) ? showModalBottomSheet(
                                            ...
                                            builder: (con) => ItemBottomSheet(item: cart.item, cartIndex: cartIndex, cart: cart),
                                              ...
                                              Dialog(child: ItemBottomSheet(item: cart.item, cartIndex: cartIndex, cart: cart),));
                                        }else if((cart.quantity! > cart.stock!)){
                                          showCustomSnackBar("This Product is Out Of Stock!");
                                        }
                                      },
